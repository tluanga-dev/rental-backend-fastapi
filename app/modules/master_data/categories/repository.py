...
class CategoryRepository:
    ...
    async def get_by_name_and_parent(
        ...
        query = select(Category).where(
            and_(Category.name == name, Category.parent_category_id == parent_id)
        )
        result = await self.session.execute(query)
        return result.scalar_one_or_none()
    ...
    async def get_children(self, parent_id: UUID) -> List[Category]:
        ...
        query = (
            select(Category)
            .where(and_(Category.parent_category_id == parent_id, Category.is_active == True))
            .order_by(Category.display_order, Category.name)
        )
        result = await self.session.execute(query)
        return result.scalars().all()
    ...
    async def get_tree(self, root_id: Optional[UUID] = None) -> List[Category]:
        ...
        if root_id:
            ...
            # Get full tree
            query = (
                select(Category).where(Category.is_active == True).order_by(Category.category_path)
            )
    ...
    async def get_paginated(
        ...
        return PaginationResult(items=items, page_info=page_info)
    ...
    async def update(self, category_id: UUID, update_data: dict) -> Optional[Category]:
        ...
        if "category_path" in update_data:
            category.update_path(
                update_data["category_path"], updated_by=update_data.get("updated_by")
            )

        if "is_leaf" in update_data:
            category.is_leaf = update_data["is_leaf"]
            category.updated_by = update_data.get("updated_by")
    ...
    async def move_category(
        self, category_id: UUID, new_parent_id: Optional[UUID], updated_by: Optional[str] = None
    ) -> Optional[Category]:
    ...
    async def exists_by_name_and_parent(
        self, name: str, parent_id: Optional[UUID] = None, exclude_id: Optional[UUID] = None
    ) -> bool:
        """Check if a category with the given name exists under the same parent."""
        query = (
            select(func.count())
            .select_from(Category)
            .where(and_(Category.name == name, Category.parent_category_id == parent_id))
        )
    ...
    async def exists_by_path(self, path: str, exclude_id: Optional[UUID] = None) -> bool:
        ...
        query = select(func.count()).select_from(Category).where(Category.category_path == path)
    ...
    async def search(
        self, search_term: str, limit: int = 10, include_inactive: bool = False
    ) -> List[Category]:
        """Search categories by name or path."""
        search_pattern = f"%{search_term}%"

        query = select(Category).where(
            or_(Category.name.ilike(search_pattern), Category.category_path.ilike(search_pattern))
        )
    ...
    async def get_leaf_categories(self) -> List[Category]:
        ...
        query = (
            select(Category)
            .where(and_(Category.is_leaf == True, Category.is_active == True))
            .order_by(Category.category_path)
        )
        result = await self.session.execute(query)
        return result.scalars().all()
    ...
    async def get_parent_categories(self) -> List[Category]:
        ...
        query = (
            select(Category)
            .where(and_(Category.is_leaf == False, Category.is_active == True))
            .order_by(Category.category_path)
        )
        result = await self.session.execute(query)
        return result.scalars().all()
    ...
    async def get_categories_by_level(self, level: int) -> List[Category]:
        ...
        query = (
            select(Category)
            .where(and_(Category.category_level == level, Category.is_active == True))
            .order_by(Category.category_path)
        )
        result = await self.session.execute(query)
        return result.scalars().all()
    ...
    async def get_categories_without_items(self) -> List[Category]:
        ...
        query = select(Category).where(Category.is_active == True).order_by(Category.category_path)
        result = await self.session.execute(query)
        return result.scalars().all()
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        root_query = (
            select(func.count())
            .select_from(Category)
            .where(and_(Category.category_level == 1, Category.is_active == True))
        )
        ...
        # Get max depth
        max_depth_query = (
            select(func.max(Category.category_level))
            .select_from(Category)
    ...
    async def update_display_orders(
        self, category_orders: List[Tuple[UUID, int]]
    ) -> List[Category]:
